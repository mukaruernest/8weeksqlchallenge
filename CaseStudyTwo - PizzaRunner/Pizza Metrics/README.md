...
``` SQL
With pizzadelivery AS 
(SELECT co.pizza_id, IF(ro.cancellation IS NULL , "delivered", "not delivered" ) AS deliverystatus, ro.order_id
...
``` SQL
WITH pizzaorders AS
(SELECT co.order_id, COUNT(co.Pizza_id) AS PizzaCount ,DENSE_RANK() OVER(ORDER BY COUNT(co.pizza_id) DESC) AS Ranked FROM customer_orders co
