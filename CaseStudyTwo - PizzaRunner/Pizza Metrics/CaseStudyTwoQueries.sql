...
SELECT COUNT(order_id) AS pizzasordered FROM customer_orders;

-- 2 How many unique customer orders were made?
SELECT COUNT(DISTINCT(order_id)) AS Uniquecustomerorders FROM customer_orders;

-- 3 How many successful orders were delivered by each runner?
SELECT runner_id, COUNT(order_id) AS DeliveredOrders FROM runner_orders
WHERE cancellation IS NULL 
GROUP BY runner_id;

-- 4 How many of each type of pizza was delivered?
With pizzadelivery AS (SELECT 
...
SELECT pn.pizza_name, COUNT(pd.deliverystatus) AS PizzaDelivered FROM pizzadelivery pd
...
WITH pizzaorders AS (SELECT co.order_id, COUNT(co.Pizza_id) AS PizzaCount ,DENSE_RANK() OVER(ORDER BY COUNT(co.pizza_id) DESC) AS Ranked FROM customer_orders co
JOIN runner_orders ro ON co.order_id = ro.order_id
WHERE cancellation IS NULL
GROUP BY order_id)
SELECT order_id, PizzaCount FROM pizzaorders po
WHERE Ranked = 1;

-- 7 For each customer, how many delivered pizzas had at least 1 change and how many had no changes?
with changedpizzas AS (SELECT co.customer_id, co.order_id ,CASE 
...
with bothchanged AS (SELECT co.order_id, CASE 
...
INNER JOIN runner_orders ro ON co.order_id = ro.order_id
WHERE cancellation IS NULL)
SELECT COUNT(*) AS PizzasDeliveredwithexclusions FROM bothchanged
WHERE ExclusionsandExtras = "BothChanges";

-- 9.What was the total volume of pizzas ordered for each hour of the day?
SELECT HOUR(order_time) AS HourofDay, COUNT(pizza_id) AS PizzaCount FROM customer_orders
GROUP BY HOUR(order_time)
ORDER BY HOUR(order_time);

-- 10 What was the volume of orders for each day of the week?
SELECT dayname(order_time) AS DayOfWeek, COUNT(pizza_id) AS PizzaCount FROM customer_orders
